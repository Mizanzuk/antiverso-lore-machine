...
async function onSubmit(e?: FormEvent) {
    ...
    try {
      ...
      // Se o backend não estiver em modo streaming por algum motivo,
      // fazemos o fallback para JSON como antes.
      if (!res.body) {
      ...
      // STREAMING: lê a resposta em pedaços e vai atualizando a última mensagem do Or
      ...
      // adiciona mensagem vazia do assistente que será preenchida aos poucos
      setSessions((prev) =>
        prev.map((s) =>
          s.id === activeSession.id
            ? {
        ...
        setSessions((prev) =>
          prev.map((s) => {
            ...
            const updatedMessages = s.messages.map((m) =>
              m.id === answerId ? { ...m, content: currentText } : m
            );
            return {
              ...s,
              messages: trimMessagesForStorage(updatedMessages),
            };
          })
        );
      }
                        ...
                        >
                          <div className="flex items-center justify-between gap-2">
                            <div className="flex-1 min-w-0">
                              ...
                              <div className="flex items-center gap-2 mt-0.5 text-[10px] text-gray-500 truncate">
                                <span className="inline-flex items-center rounded-full border border-white/20 px-1.5 py-[1px] text-[9px] uppercase tracking-wide text-gray-400">
                                  {session.mode === "criativo" ? "CRIATIVO" : "CONSULTA"}
                                </span>
