...
function LoreAdminContent() {
  ...
  return (
    <div className="h-screen bg-black text-neutral-100 flex flex-col overflow-hidden font-sans">
      ...
      {universeFormMode !== "idle" && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setUniverseFormMode("idle")}><form onSubmit={e => { e.preventDefault(); saveUniverse(); }} onClick={(e) => e.stopPropagation()} className="bg-zinc-950 border border-zinc-800 p-6 rounded-lg max-w-md w-full mx-4 shadow-xl"><div className="flex items-center justify-between mb-4"><h3 className="text-white font-bold text-sm">{universeFormMode === 'create' ? 'Novo Universo' : 'Editar Universo'}</h3><button type="button" onClick={() => setUniverseFormMode("idle")} className="text-zinc-500 hover:text-white text-2xl leading-none font-light">&times;</button></div><input className="w-full bg-black border border-zinc-800 rounded p-2 mb-2 text-sm text-white" placeholder="Nome" value={universeForm.nome} onChange={e=>setUniverseForm({...universeForm, nome: e.target.value})} /><textarea className="w-full bg-black border border-zinc-800 rounded p-2 mb-4 text-sm text-white h-20" placeholder="Descrição" value={universeForm.descricao || ""} onChange={e=>setUniverseForm({...universeForm, descricao: e.target.value})} /><div className="flex justify-end gap-2"><button type="button" onClick={() => setUniverseFormMode("idle")} className="text-xs text-zinc-400 hover:text-zinc-100">Fechar</button><button className="bg-emerald-600 hover:bg-emerald-500 text-white px-4 py-2 rounded text-xs font-bold">Salvar</button></div></form></div>)}
      {worldFormMode !== "idle" && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setWorldFormMode("idle")}><form onSubmit={handleSaveWorld} onClick={(e) => e.stopPropagation()} className="w-full max-w-md max-h-[90vh] overflow-auto border border-zinc-800 rounded-lg p-6 bg-zinc-950 space-y-3 mx-4"><div className="flex items-center justify-between"><h3 className="text-white font-bold text-sm">{worldFormMode === 'create' ? 'Novo Mundo' : 'Editar Mundo'}</h3><button type="button" onClick={() => setWorldFormMode("idle")} className="text-zinc-500 hover:text-white text-2xl leading-none font-light">&times;</button></div><div className="space-y-1"><label className="text-[11px] text-zinc-500">Nome</label><input className="w-full rounded-md bg-zinc-900 border border-zinc-700 px-3 py-2 text-sm" value={worldForm.nome || ""} onChange={e=>setWorldForm({...worldForm, nome: e.target.value})} placeholder="Ex: Arquivos Vermelhos" /></div><div className="space-y-1"><label className="text-[11px] text-zinc-500">Descrição</label><textarea className="w-full rounded-md bg-zinc-900 border border-zinc-700 px-3 py-2 text-sm min-h-[140px]" value={worldForm.descricao || ""} onChange={e=>setWorldForm({...worldForm, descricao: e.target.value})} placeholder="Resumo do Mundo…" /></div><div className="flex items-center gap-2 pt-1"><button type="button" onClick={() => setWorldForm({...worldForm, has_episodes: !worldForm.has_episodes})} className={`h-4 px-2 rounded border text-[11px] ${worldForm.has_episodes ? "border-emerald-400 text-emerald-300 bg-emerald-400/10" : "border-zinc-700 text-zinc-400 bg-black/40"}`}>Este mundo possui episódios</button></div><div className="flex justify-end gap-2 pt-1"><button type="button" onClick={() => setWorldFormMode("idle")} className="text-xs text-zinc-400 hover:text-zinc-100">Fechar</button><button type="submit" className="px-3 py-1.5 rounded bg-emerald-600 hover:bg-emerald-500 text-[11px] font-medium">Salvar</button></div></form></div>)}
