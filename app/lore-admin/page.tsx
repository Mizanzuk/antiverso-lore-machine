...
function LoreAdminContent() {
  ...
  return (
    <div className="h-screen bg-black text-neutral-100 flex flex-col overflow-hidden font-sans">
      ...
      {worldFormMode !== "idle" && (<div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm"><form onSubmit={handleSaveWorld} className="bg-zinc-950 border border-zinc-800 p-6 rounded-lg w-96 shadow-xl"><h3 className="text-white font-bold mb-4">{worldFormMode === 'create' ? 'Novo Mundo' : 'Editar Mundo'}</h3><input className="w-full bg-black border border-zinc-800 rounded p-2 mb-2 text-sm text-white" placeholder="Nome" value={worldForm.nome || ""} onChange={e=>setWorldForm({...worldForm, nome: e.target.value})} /><textarea className="w-full bg-black border border-zinc-800 rounded p-2 mb-4 text-sm text-white h-20" placeholder="Descrição" value={worldForm.descricao || ""} onChange={e=>setWorldForm({...worldForm, descricao: e.target.value})} /><div className="flex items-center gap-2 mb-4"><input type="checkbox" id="has_episodes" checked={worldForm.has_episodes || false} onChange={e=>setWorldForm({...worldForm, has_episodes: e.target.checked})} className="w-4 h-4" /><label htmlFor="has_episodes" className="text-white text-sm">Este mundo possui episódios</label></div><div className="flex justify-end gap-2"><button type="button" onClick={() => setWorldFormMode("idle")} className="text-zinc-400 text-xs">Cancelar</button><button className="bg-emerald-600 text-white px-4 py-2 rounded text-xs font-bold">Salvar</button></div></form></div>)}
